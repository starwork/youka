<template>
  <div class="yg-order">
    <van-nav-bar class="yg-nav-bar" title="下级订单" left-arrow @click-left="onClickLeft"/>
          ...
          <div v-if="hasData" class="order-list">
            <div class="item" v-for="(item, index) in orderList_0" :key="index">
              <van-row class="status">
                <van-col class="status-left" span="12">
                    <div class="van_img">
                        <img :src="item.user.avatarUrl" alt="">
                    </div>
                    <p class="van_p">{{item.user.nickName}}</p>
                </van-col>
								<van-col v-if="item.pay_status.value == 10 && item.order_status.value == 10"  class="status-right" span="12">等待付款</van-col>
								<van-col v-if="item.pay_status.value == 20 && item.order_status.value == 10 && item.delivery_status.value == 10"  class="status-right" span="12">等待卖家发货</van-col>
								<van-col v-if="item.delivery_status.value == 20 && item.order_status.value == 10"  class="status-right" span="12">卖家已发货</van-col>
								<van-col v-if="item.order_status.value == 30"  class="status-right" span="12">交易成功</van-col>
              </van-row>
              <van-card
                desc=""
                 :title="goods_title(item.goods)"
                :thumb="goods_thumb(item.goods)"
								@click="toDeatil(item.order_id)"
              >
                <div slot="footer" class="custom-card-footer">
                  <div class="order-info">
                    共{{item.total_num}}件商品 合计：<span class="price">￥{{item.total_price}}</span>
                  </div>
                </div>
              </van-card>
            </div>
...
<script>
import {getChildOrder} from '../../api/order'
...
export default {
  ...
  methods: {
    ...
    getOrderList(type, active) {
      const self = this;
      self.loading = true;
      getChildOrder(type).then(response => {
        if(active === 0) {

...
<template>
    <div class="orderManager">
        <van-nav-bar class="yg-nav-bar" title="订货管理" left-arrow @click-left="onClickLeft"/>
    <!-- 图表 -->
    <div class="vol-data">
      <div class="charts" style="width:'100%',height:'6.54rem'">
        ...
        <div ref="charts" id="myChart" :style="{width:'100%',height:'300px'}">
        </div>
      </div>
    </div>
    <div class="wealthLink" ref="footer">
        <div @click="goToPage('/user/orderXiadan')" class="w-banner1">
              <img src="../../assets/images/order/1.png" alt="">
              <p>订货下单</p>
          </div>
          <div @click="goToPage('/user/order?active=0')" class="w-banner1">
              <img src="../../assets/images/order/2.png" alt="">
              <p>我的订单</p>
          </div>
          <div @click="goToPage('/user/suborder?active=0')" class="w-banner1">
              <img src="../../assets/images/order/3.png" alt="">
              <p>下级订单</p>
          </div>
    </div>
    </div>
...
export default {
  ...
  methods:{
	...
	mounted () {
	 ...
	 this.$refs.charts.style.height = window.innerHeight-20-top-footer_height+'px';
	 self.changeCharts();
	 setTimeout(function(){
	 	self.myChart.resize();
	 },200)
